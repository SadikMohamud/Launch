    ...
    <header className="border-b-2 border-ink-900 bg-white/95 backdrop-blur-xl sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          ...
          <div>
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className="font-sans font-extrabold text-xl sm:text-2xl text-ink-900 tracking-tight">Launch</span>
        ...
        <div className="hidden md:flex items-center gap-6">
          <nav className="flex items-center gap-6 text-sm font-mono font-bold text-ink-900">
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl bg-surface border-2 border-ink-900 flex items-center justify-center text-ink-900 shadow-sm"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-b-2 border-ink-900 bg-surface/98 backdrop-blur-2xl px-6 py-6 space-y-5 animate-in slide-in-from-top-4 duration-200">
          <nav className="flex flex-col space-y-3 font-mono text-base font-extrabold text-ink-900">
            <button
              onClick={() => handleNavClick('showcase')}
              className="flex items-center justify-between p-3 rounded-xl bg-white border-2 border-ink-900 shadow-sm hover:bg-accent-yellow transition-colors text-left"
            >
